...
export default {
  ...
  methods: {
        ...
        if (scoreBoard.length > 0) {
          scoreBoard.forEach((info, i) => {
            let rowObj = {
              ...
              Problems: schedule.find((e) => e.id === info.schedule_id).title,
            ...
            if (i === scoreBoard.length - 1) {
              ...
              rowObj.countSubmit = scoreBoard.reduce((t, n) => t + n.count, 0);
    ...
    //     query: { course_id: item.course_id, user_id: item.user_id },
    //   });
    // },
    fetchRoom(val) {
      this.$router
